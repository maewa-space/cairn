...
- **130 unit tests** pass (vitest) — was 83
- **24 mocked e2e** pass (playwright + electron) — was 21
- **3 DMG smoke** pass against the packaged app
- **typecheck** clean (`tsc --noEmit` on both `tsconfig.node.json` and `tsconfig.web.json`)
- **production build** clean (`pnpm build`; ~1.30 MB JS, ~52 KB CSS, fonts self-hosted as woff2 chunks)
...
## What shipped today (2026-05-09, fourth session)

### LLM-based auto-pick template

The biggest UX win of the day. The Enhance dropdown now starts in **Auto**
mode — when you click Enhance, the cheapest configured LLM picks the
best-fitting template based on what was actually said.

Previously the dropdown auto-bootstrapped to the alphabetically-first
template (1-on-1), so non-1-on-1 content (tech rants, random conversations)
was getting forced into a Manager/Direct-report frame. Output quality
suffered.

- `src/main/services/template-picker.ts` — `pickTemplate(transcript,
  notes, templates, keys) -> templateId`. Same provider order as enhancer
  (OpenRouter → Anthropic → OpenAI), 64-token output cap, fallback to
  "generic" when no LLM key or model returns junk. Iterative sanitizer
  peels nested decorations like `"standup".` → `standup`.
- `src/shared/template-picker.ts` — `TemplateChoice` descriptor (id,
  name, description) — minimal slice of Template the picker needs.
- `templates:autoPick` IPC + `window.quill.templates.autoPick` bridge.
- `EnhanceBar.tsx` — removed bootstrap useEffect, added "Auto" synthetic
  option (with Wand2 icon) at top of dropdown. Button reads "Auto" when
  templateId=null. `onSelect` widened to accept null.
- `meeting.tsx runEnhance` — when templateId is null, calls autoPick
  first; falls back to templates[0] if that errors. Visible error
  banner when both paths fail.
- 10 new unit tests on the picker (cleanup, fallbacks, provider order).

### Smarter auto-titling — timestamp default + post-Enhance refinement

"Untitled meeting" replaced with `Mon May 12 · 14:30` so every row in
the sidebar is distinguishable even before any LLM titling has run.
LLM-derivation fires on Stop AND after Enhance (using polished
`enhancedNotes` as the strongest signal).

- New `meetings.title_is_auto` column tracks whether the title was
  generated by Quill (timestamp / calendar-matched / LLM-derived) or
  typed manually. Migration backfills existing rows: `Untitled meeting`
  → 1, anything else → 0 (preserve user-typed titles).
- `src/shared/meeting-title.ts` — `defaultMeetingTitle()` renders the
  timestamp; `looksLikeAutoTitle()` regex-detects auto patterns.
- Manual rename clears the flag (locking the title in). `runEnhance`
  fires `meetings.autoTitle(id)` on success — uses `enhancedNotes` if
  present (sharper than raw transcript).
- Provider order: OpenRouter → Anthropic → OpenAI (same as enhancer).
- 7 new unit tests on the helpers + 1 on `enhancedNotes` priority.

### Silent Enhance click — fixed

Clicking "Enhance notes" did nothing on fresh meetings because the
meeting load was clobbering EnhanceBar's bootstrap `templateId` state
with the persisted `null`, causing `runEnhance` to silently early-return.
The button visually showed a template name because EnhanceBar fell back
to `templates[0]` for *display*, but state was actually null.

- `meeting.tsx` load: `setTemplateId((cur) => m.templateId ?? cur)` —
  only override when the meeting actually has one persisted.
- `runEnhance` JIT fallback: if state is still null at click time,
  fetch the list and pick `[0]`. Surfaces a visible error if no
  templates exist at all.
- `alert()` replaced with the inline editorial banner pattern (same
  shape as `notesError`, dismissible, `testid="enhance-error"`).
- Regression test: `tests/e2e/enhance-click.spec.ts` seeds a meeting
  with no LLM key, clicks Enhance, asserts the catch path fires.

### Transcript-based auto-titling (initial pass)

(Superseded by smarter version above, but documented here because the
infrastructure stuck around.) `src/main/services/title.ts` — derives
3-6 word titles via cheapest configured LLM. Cleaned to strip stray
quotes / trailing periods / multi-line noise; ≤80 chars or rejected.
`meetings:autoTitle` IPC fires on Stop recording.

### Readable chat dates

`chat.ts` was passing `meeting.startedAt` as raw ISO into the LLM
context, so answers like "Your last meeting was on
2026-05-08T15:50:26.000Z" surfaced verbatim. New `formatHumanDate`
renders "Fri, May 8 2026 · 15:50" and the LLM echoes that instead.

### Maintenance script

`scripts/cleanup-all-meetings.mjs` — one-shot wipe of every meeting in
the local DB. Useful for clearing test data without uninstalling. Note:
better-sqlite3 native bindings are compiled against electron's Node ABI;
running via system Node fails with `NODE_MODULE_VERSION` mismatch. The
script is a reference implementation; the actual cleanup ran via
`sqlite3` CLI.

### Comprehensive UI audit

`tests/e2e/ui-audit.spec.ts` walks home / settings / templates / global
chat / meeting / folder-filtered / onboarding, captures full-page
screenshots at 1440×900, asserts no document overflow + no console
errors, and exercises the new transcription controls + meeting action
menu. Artifacts under `test-results/ui-audit/`. All 9 screenshots clean.

---

## What shipped earlier today (2026-05-09, third session)
