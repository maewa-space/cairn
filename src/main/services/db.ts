...
function migrate(d: Database.Database): void {
  ...
  for (const m of POST_SCHEMA_MIGRATIONS) {
    ...
    if (!cols.some((c) => c.name === m.column)) {
      d.exec(m.ddl);
      if (m.backfill) d.exec(m.backfill);
    }
  }
}
...
interface MeetingRow {
  ...
  attendees: string;
  /** 1 when the title was generated by Quill (timestamp default,
   *  calendar-matched, or LLM-derived); 0 when the user typed it manually.
   *  Auto-titling only overwrites titles where this is 1. */
...
function toMeeting(row: MeetingRow, transcript: TranscriptEntry[]): Meeting {
  ...
  return {
    ...
    attendees,
    // Older rows (before this column existed) come back with NaN/undefined
    // out of better-sqlite3; fall back to true so they're considered auto
    // and can be refined by Enhance.
...
export interface CreateMeetingOptions {
  ...
  attendees?: string[];
  /** Whether the supplied title was generated by Quill (timestamp,
   *  calendar-matched, or LLM-derived). Defaults to true. Pass false when
   *  the user explicitly typed the title (e.g. via the New-meeting dialog). */
  titleIsAuto?: boolean;
}
...
export const meetingsRepo = {
  create(title: string, options: CreateMeetingOptions = {}): Meeting {
    ...
    const titleIsAuto = options.titleIsAuto === false ? 0 : 1;
    d.prepare(
      `INSERT INTO meetings (id, title, started_at, raw_notes, calendar_event_id, attendees, title_is_auto)
       VALUES (?, ?, ?, '', ?, ?, ?)`,
    ).run(
      id,
      title,
  ...
  rename(
    id: string,
    title: string,
    options: { titleIsAuto?: boolean } = {},
  ): void {
    // Default: a rename clears the auto flag (user-typed). Auto-titling
    // call sites pass titleIsAuto: true so the flag stays on, allowing a
    // future Enhance to refine again.
    const titleIsAuto = options.titleIsAuto === true ? 1 : 0;
    getDb()
      .prepare(`UPDATE meetings SET title = ?, title_is_auto = ? WHERE id = ?`)
      .run(title, titleIsAuto, id);
  },
