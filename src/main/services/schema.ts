...
export const POST_SCHEMA_MIGRATIONS: Array<{
  ...
  column: string;
  ddl: string;
  /** Optional one-shot SQL run once *after* the column is created. Used to
   *  back-fill a sensible default for existing rows when the column's
   *  static DEFAULT doesn't carry the right semantics. */
  backfill?: string;
}> = [
  {
    table: 'meetings',
  ...
  {
    // Marks whether the title was generated by Quill (timestamp default,
    // calendar-matched, or LLM-derived) vs typed manually by the user.
    // Auto-titling only overwrites titles where this is 1.
    table: 'meetings',
    column: 'title_is_auto',
    ddl: `ALTER TABLE meetings ADD COLUMN title_is_auto INTEGER NOT NULL DEFAULT 1`,
    // For pre-existing rows: only "Untitled meeting" is treated as auto;
    // anything else (calendar matches, user-typed) is locked in so we
    // don't accidentally clobber a meaningful existing title on Enhance.
    backfill: `UPDATE meetings SET title_is_auto = 0 WHERE title != 'Untitled meeting'`,
  },
];
